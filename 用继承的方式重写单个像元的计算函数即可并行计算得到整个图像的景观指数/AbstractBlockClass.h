...
namespace CuLSM
{
	...
	public:
		...
		void freeHoloDown();
		void freeCompactSrc();
		void getCompactRelabel(UnionFind<Patch,ADD<int>>* Quf);	//得到原始标记值对应的连续标记值
		void relabelLabelVal();		//将mh_LabelVal中的值修改为连续的标记值,并行
		void relabelLabelVal(UnionFind<Patch,ADD<int>>* Quf);		//串行，用union-find查找，快很多
